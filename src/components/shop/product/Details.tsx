...
export const Details: React.FC<Props> = ({ meta, activeEdition, editionChange }) => {
  ...
  return (
    <div
      className="flex flex-col relative p-3.5 sm:p-5 lg:p-7 xl:p-8 mt-0 lg:mt-0 w-full max-w-none border border-black/10 shadow-[0_10px_30px_rgba(0,0,0,0.18)]"
      style={{ background: "rgba(255, 255, 255, 0.96)" }}
    >
      <div className="flex flex-col mb-4">
        <h1
          className="text-[22px] sm:text-[25px] md:text-[28px] font-bold tracking-tighter leading-tight text-black mb-3 uppercase
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-3 gap-3">
          <span className="text-[9px] sm:text-[10px] md:text-[10px] font-medium text-black uppercase tracking-widest">
            Ref: {meta?.model_id || meta?.id_producto || "3D-DC"}
          </span>

          <span className="text-[16px] sm:text-[18px] md:text-[18px] font-light text-black whitespace-nowrap">
            {priceLabel ?? ""}
          </span>
        </div>
      </div>
      ...
      <div className="mb-5">
        <p className="text-[10px] sm:text-[10px] md:text-[10px] font-black uppercase tracking-[1.8px] mb-3 text-black">
          Versión del modelo
        </p>
        ...
        <div className="grid grid-cols-1 gap-1.5">
          {editions.length ? (
              ...
              return (
                <button
                  ...
                  className={classNames(
                    "relative flex items-center justify-between px-3 py-2 border text-[10px] sm:text-[11px] md:text-[11px] transition-all duration-300 uppercase tracking-tight font-bold text-left",
                    isActive
            ...
            <div className="text-[11px] sm:text-[12px] text-zinc-500 italic">
      ...
      {scales.length > 0 && (
        <div className="mb-6">
          ...
          {activeScaleDescription && (
            <div className="text-[11px] sm:text-[12px] md:text-[12px] italic text-zinc-700 leading-relaxed mb-4 whitespace-pre-line">
              {activeScaleDescription}
            </div>
          )}
            ...
            {scales.map((scale, index) => {
              ...
              return (
                <button
                  ...
                  className={classNames(
                    "min-w-[42px] sm:min-w-[46px] px-2.5 sm:px-3 py-2.5 border text-[9px] sm:text-[10px] md:text-[10px] font-bold uppercase leading-none transition-all duration-300",
                    !scale?.disponible
      ...
      <div className="flex flex-col gap-2 mb-6">
        <button
          ...
          className={classNames(
            "w-full py-3 sm:py-3.5 font-bold uppercase text-[9px] sm:text-[10px] md:text-[10px] tracking-[2px] border transition-colors",
            isFavorite
      ...
      <div className="border-t border-zinc-200 pt-4">
        {meta?.aspectos_variables ? (
          <div className="mb-4 rounded-sm border-l-4 border-red-500 bg-red-50 px-3 sm:px-4 py-3 shadow-sm">
            <div className="text-[11px] sm:text-[13px] md:text-[14px] italic leading-relaxed tracking-tight text-black">
        ...
        <div className="text-black font-sans italic text-[11px] sm:text-[12px] md:text-[12px] leading-relaxed tracking-tight whitespace-pre-line">
          {meta?.description || meta?.descripcion || "Sin descripción por el momento."}
        </div>

        {meta?.disclaimer ? (
          <div className="text-black/60 font-sans italic text-[9px] sm:text-[10px] md:text-[10px] leading-relaxed tracking-tight whitespace-pre-line mt-4">
            {meta.disclaimer}
          </div>
        ) : null}
      </div>
