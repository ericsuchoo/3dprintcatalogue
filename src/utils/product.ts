...
// ✅ ahora exige que le pases el cliente (runtime o build)
export const productUtils = {
  all: async (bcms: Client): Promise<ProductLite[]> => {
    try {
      const entries = (await bcms.entry.getAll("product")) as ProductEntry[];
      if (!entries) return [];
