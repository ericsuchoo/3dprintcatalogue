...
export const Details: React.FC<Props> = ({ meta, activeEdition, editionChange }) => {
  ...
  return (
    <div
      className="flex flex-col relative p-4 sm:p-6 lg:p-8 xl:p-9 mt-0 lg:mt-0 w-full max-w-none border border-black/10 shadow-[0_10px_30px_rgba(0,0,0,0.18)]"
      style={{ background: "rgba(255, 255, 255, 0.96)" }}
    >
      <div className="flex flex-col mb-5">
        <h1
          className="text-[26px] sm:text-[28px] md:text-3xl font-bold tracking-tighter leading-tight text-black mb-4 uppercase
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-4 gap-3">
          <span className="text-[10px] md:text-[10px] font-medium text-black uppercase tracking-widest">
            Ref: {meta?.model_id || meta?.id_producto || "3D-DC"}
          </span>

          <span className="text-[20px] md:text-[20px] font-light text-black whitespace-nowrap">
            {priceLabel ?? ""}
          </span>
        </div>
      </div>
      ...
      <div className="mb-6">
        <p className="text-[10px] md:text-[10px] font-black uppercase tracking-[2px] mb-3 text-black">
          Versión del modelo
        </p>
        ...
        <div className="grid grid-cols-1 gap-2">
          {editions.length ? (
              ...
              return (
                <button
                  ...
                  className={classNames(
                    "relative flex items-center justify-between px-3.5 sm:px-4 py-2.5 border text-[12px] md:text-[11px] transition-all duration-300 uppercase tracking-tight font-bold text-left",
                    isActive
            ...
            <div className="text-[12px] md:text-[12px] text-zinc-500 italic">
      ...
      {scales.length > 0 && (
        <div className="mb-8">
          ...
          {activeScaleDescription && (
            <div className="text-[12px] md:text-[12px] italic text-zinc-700 leading-relaxed mb-4 whitespace-pre-line">
              {activeScaleDescription}
            </div>
          )}
            ...
            {scales.map((scale, index) => {
              ...
              return (
                <button
                  ...
                  className={classNames(
                    "min-w-[46px] px-3 py-3 border text-[10px] md:text-[11px] font-bold uppercase leading-none transition-all duration-300",
                    !scale?.disponible
      ...
      <div className="flex flex-col gap-2 mb-8">
        <button
          ...
          className={classNames(
            "w-full py-3.5 sm:py-4 font-bold uppercase text-[10px] md:text-[10px] tracking-[2px] border transition-colors",
            isFavorite
      ...
      <div className="border-t border-zinc-200 pt-5">
        {meta?.aspectos_variables ? (
          <div className="mb-5 rounded-sm border-l-4 border-red-500 bg-red-50 px-3.5 sm:px-4 py-3.5 sm:py-4 shadow-sm">
            <div className="text-[13px] md:text-[16px] italic leading-relaxed tracking-tight text-black">
        ...
        <div className="text-black font-sans italic text-[13px] md:text-[13px] leading-relaxed tracking-tight whitespace-pre-line">
          {meta?.description || meta?.descripcion || "Sin descripción por el momento."}
        </div>

        {meta?.disclaimer ? (
          <div className="text-black/60 font-sans italic text-[10px] md:text-[10px] leading-relaxed tracking-tight whitespace-pre-line mt-4">
            {meta.disclaimer}
          </div>
        ) : null}
      </div>
