    ...
    >
      <div className="flex flex-col mb-5">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 pb-5 gap-4">
          <span className="text-[12px] md:text-[12px] font-medium text-black uppercase tracking-widest">
            Ref: {meta?.model_id || meta?.id_producto || "3D-DC"}
          </span>

          <span className="text-[24px] md:text-[20px] font-light text-black whitespace-nowrap">
            {priceLabel ?? ""}
          </span>
        </div>
      </div>
      ...
      <div className="mb-7">
        <p className="text-[11px] md:text-[12px] font-black uppercase tracking-[2px] mb-4 text-black">
          Versión del modelo
        </p>
              ...
              return (
                <button
                  ...
                  className={classNames(
                    "relative flex items-center justify-between px-4 py-2.5 border text-[14px] md:text-[14px] transition-all duration-300 uppercase tracking-tight font-bold text-left",
                    isActive
      ...
      {scales.length > 0 && (
        <div className="mb-9">
          <p className="text-[13px] md:text-[16px] font-black uppercase tracking-[2px] mb-4 text-black">
            Escala disponible
          </p>

          {activeScaleDescription && (
            <div className="text-[13px] md:text-[16px] italic text-zinc-700 leading-relaxed mb-5 whitespace-pre-line">
              {activeScaleDescription}
            </div>
          )}
            ...
            {scales.map((scale, index) => {
              ...
              return (
                <button
                  ...
                  className={classNames(
                    "min-w-[50px] px-3.5 py-3 border text-[13px] md:text-[16px] font-bold uppercase leading-none transition-all duration-300",
                    !scale?.disponible
      ...
      <div className="flex flex-col gap-2 mb-9">
        <button
          ...
          className={classNames(
            "w-full py-4 font-bold uppercase text-[13px] md:text-[16px] tracking-[2px] border transition-colors",
            isFavorite
      ...
      <div className="border-t border-zinc-200 pt-6">
        {meta?.aspectos_variables ? (
          <div className="mb-6 rounded-sm border-l-4 border-red-500 bg-red-50 px-4 py-4 shadow-sm">
            <div className="text-[16px] md:text-[19px] italic leading-relaxed tracking-tight text-black">
        ...
        <div className="text-black font-sans italic text-[14px] md:text-[18px] leading-relaxed tracking-tight whitespace-pre-line">
          {meta?.description || meta?.descripcion || "Sin descripción por el momento."}
        </div>

        {meta?.disclaimer ? (
          <div className="text-black/60 font-sans italic text-[13px] md:text-[16px] leading-relaxed tracking-tight whitespace-pre-line mt-5">
            {meta.disclaimer}
          </div>
        ) : null}
      </div>
